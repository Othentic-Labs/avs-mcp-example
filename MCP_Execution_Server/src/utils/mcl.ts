...
export function getSigningKey(seed: any) {
    return parseFr(prefixSeed(seed));
}

function getPublicKey(signingKey: any) {
    const pubKey = mcl.mul(g2(), signingKey);
    pubKey.normalize();
    return pubKey; 
}

export function sign(signingKey: any, message: any) {
...
function prefixSeed(seed: any) {
    return seed.startsWith('0x') ? seed : `0x${seed}`;
}

function hashToPoint(message: any, domain: any) {
...
function mapToPoint(e: any) {
    const e1 = new mcl.Fp();
    e1.setStr((ethers.toBigInt(e) % FIELD_ORDER).toString());
    return e1.mapToG1();
}

function hashToField(domain: any, msg: any, count: any) {
    const u = 48;
...
function toBigEndian(p: any) {
    return p.serialize().reverse();
}

function g1ToHex(p: any) {
    p.normalize();
...
function expandMsg(domain: any, msg: any, outLen: any) {
    if (domain.length > 32) {
...
function bytes32ArrayZeroPadding(array: any) {
